...
COPERNICUS = Path("~/copernicus_data")
TCACHE = Path("data")
CACHE = Path("cache")
...
def load_extfield(years: list | int | None = None) -> diffsys.ExternalField:
    ...
    if isinstance(years, int):
        data = xr.load_dataarray(
            COPERNICUS / f"IT_total_precipitation_land/total_precipitation_IT_{years}.nc",
            decode_coords="all",
        )
    else:
        data = xr.concat(
            [
                xr.load_dataarray(
                    COPERNICUS / f"IT_total_precipitation_land/total_precipitation_IT_{year}.nc",
                    decode_coords="all",
                )
                for year in years
            ],
...
def load_nodes() -> geopd.GeoDataFrame:
    return geopd.read_file(TCACHE / "graph_nodes_metadata.geojson").set_index("index", drop=True)
...
def load_graph(full: bool = True, days: pd.DatetimeIndex | None = None) -> diffsys.Graph:
    ...
    transitions = pd.read_csv(TCACHE / "aggregate_transitions_learn_ita.csv.gz", index_col=0).drop(
        columns=["cumul", "type"]
    )
    ...
    if full:
        ...
        transitions = (
            transitions.groupby(["start", "end", "month", "weekday", "hour"]).sum().reset_index()
        )
...
def params(
    ...
    if path is None:
        path = sorted((CACHE / "optimize_pars").glob("*KFOLD*"))
    ...
    if isinstance(path, list):
        ...
        if df == "ci":
            ...
            for _, vals in pars.items():
                bs = stats.bootstrap([vals], aggr)
                out.append(
                    {
                        "low": bs.confidence_interval[0],
                        "high": bs.confidence_interval[1],
                        "stats": aggr(bs.bootstrap_distribution),
                    }
                )
...
def load_rain(year: int = 2020, clip: shapely.MultiPolygon | shapely.Geometry | None = None):
    rain = xr.load_dataarray(
        COPERNICUS / f"IT_total_precipitation_land/total_precipitation_IT_{year}.nc"
    ).rio.write_crs("4326")
    if clip is not None:
        rain = rain.rio.clip([clip])
    rain = rain.sum(dim=("latitude", "longitude")).drop_vars(["number", "spatial_ref"])
    return rain.resample(valid_time="1D").sum().to_dataframe().rename(columns={"tp": "rain"})


def load_peaks(year: int | list[int] | None = None, kind: str = "both") -> pd.DataFrame:
    """Load the peaks."""
    data: pd.DataFrame = pd.read_csv(TCACHE / "rainy_peaks.csv.gz", index_col=0, parse_dates=True)
    if kind == "both":
...
def sim(
    model: Diffusion, full_graph: diffsys.Graph, usecache: bool = True, cached: dict | None = None
) -> pd.DataFrame:
    """Simulate a day.
    ...
    _full_graph = full_graph.subset([("weekday", day.day_of_week == 6), ("month", day.month == 8)])
    ...
    for ev_hour in model.ex_field.extreme_events(kind="simple"):
        ...
        if hour not in LOC_CACHE:
            ...
            _delays_ = generated_delay(
                hourly_graph, ev_hour, trange=hour, weight="count", integral=integral
            )
            if usecache:
...
def load_real_delay(year: int | list[int] | None):
    ...
    if isinstance(year, int):
        return pd.read_csv(
            TCACHE / f"delays_per_stations_{year}.csv.gz", parse_dates=["time"], index_col="time"
        )
